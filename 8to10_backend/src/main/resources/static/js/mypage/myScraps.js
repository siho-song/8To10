...
function showBoard() {
    ...
    // 게시글 렌더링
    boardContent.innerHTML = renderPosts(myScrapData.slice(0, myScrapData));
    updatePagination(myScrapData.length, postsPerPage);
}
...
function updatePagination(totalPosts, postsPerPage) {
    ...
    if (pageCount > 0) {
        ...
        document.getElementById('board-content').innerHTML = renderPosts(myScrapData.slice(start, end));
    }
}
