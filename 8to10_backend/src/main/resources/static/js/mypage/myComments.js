const myCommentedData = [
    {
        "board_id": 1,
        "title": "내가 댓글 쓴 글 1",
        "content": "내가 댓글 쓴 첫 번째 글 내용입니다.",
        "created_by": "다른사람 닉네임",
...
function showBoard() {
    ...
    // 게시글 렌더링
    boardContent.innerHTML = renderPosts(myCommentedData.slice(0, myCommentedData));
    updatePagination(myCommentedData.length, postsPerPage);
}
...
function updatePagination(totalPosts, postsPerPage) {
    ...
    if (pageCount > 0) {
        ...
        document.getElementById('board-content').innerHTML = renderPosts(myCommentedData.slice(start, end));
    }
}
